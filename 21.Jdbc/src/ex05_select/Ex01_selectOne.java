...
public class Ex01_selectOne {
  ...
  public static void main(String[] args) {
    ...
    try {
      //Connection 객체 생성(DB Connect 클래스의 getConnection 메소드로 부터 받아오기)
      ...
      ps = con.prepareStatement(sql); // 쿼리문의 결과가 나오게 된다.
      
      // 쿼리문에 변수 넣기
      ps.setInt(1, user_no);  // 1번째 물음표에 유저 넘버 넣어달라 
      
      // 쿼리문 실행
      rs = ps.executeQuery();  //executeQuery 와 executeUpdate 구별해서 공부하기 
    
      //검색 결과를 저장할 UserDto 객체 선언
      UserDto user = null;
      
      //검색결과 행이 1개이니 경우 if문을 이용해서검색 결과가 존재하는지 1번만 체크한다. 
      if(rs.next()) {
       ...
       * |-------------------|---------------|-----------|-----------|
       *  rs.getInt("USER_NO")   rs.getString("USER_ID")      rs.getDate("JOINED_AT")
       *  re.getInt(1)           rs.getString(2)                                 
       *                                               
       */
    } catch(Exception e) {
      e.printStackTrace();

...
public class Ex01_String {
  ...
  public static void ex03() {
    ...
    boolean isEqual3 = name1.equalsIgnoreCase(name3); // IgnoreCase --> 대소문자 무시하겠다 /대소문자를 무시하고 문자열을 비교 
  ...
  public static void ex05() {
    ...
    System.out.println(length); // 빈공간도 문자열에 포함
  ...
  public static void ex07() {
    ...
    // 2. substring(int begin, int end) : 인덱스 begin부터 end 이전따지 반환(begin <== 추출범위< end)
  ...
  public static void ex08() {
    ...
    // indexOf 메소드
    // 1.특정 문자열의 인덱스를 반환
    // 2.특정 문자열이 2개 이상 존재하는 경우 발견된 첫번째 문자열의 인덱스를 반환
    // 3.발견되지 않으면 -1을 반환
    
    // indexOf 사용법
    // 1. indexOf(String str) : 인뎃스 0부터 String str을 검색
  ...
  public static void ex12() {
    ...
    // 숫자 형식
    int number = 100; //10 진수 100
    System.out.println(String.format("%o", number)); //%o : 8진수
    System.out.println(String.format("%d", number)); //%d : 10진수
    System.out.println(String.format("%x", number)); //%x : 16진수(0 ~ 9,a ~ f)
    System.out.println(String.format("%X", number)); //%X : 16진수(0 ~ 9,A ~ F) 대문자로 16진수 표기 하겠다.
  ...
  public static void ex15() {
    ...
    // 정규식을 활용한 메소드 
    // 1. startsWith  : 지정된 규식 패턴으로 시작하면 true 반환
    // 2. endsWith    : 지정된 정규식 패턴으로 끝나면 true 반환
    // 3. matches     : 지정된 정규식 패턴을 포함하면 true 반환
    // 4. relaceAll   : 지정된 정규식 패턴을 만족하는 부분을 다른 문자열로 반환 
    // 5. split       : 지정된 정규식 패턴으로 문자열을 분리하여 분리된 String 배열을 반환
    
    String ip = "192.168.0.101";
    
    String str = ip.replaceAll(".", "_");  // 192_168_0_101 을 기대하지만 정규식 패턴에서  마침표는 모든 문자를 의미한다. 
    System.out.println(str);

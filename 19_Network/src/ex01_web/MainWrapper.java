...
public class MainWrapper {
  ...
  private static void ex03() {
    ...
    //입력 스트림의 선언(네이버 배너를 읽는 스트림)
    BufferedInputStream bin = null;
     
    //출력 스트림 선언(C:/storage/banner.jpeg 파일을 만드는 스트림)
   ...
   try{
     ...
     // 읽은 데이터를 저장할 바이트 배열
     byte[] b = new byte[1024];  //1kB 씩 읽기 
     
     // 실제로 읽은 바이트 수가 필요하다 
     int readByte = 0;
          
     // 읽기(네이버 배너의 이미지를 byte[] b에 저장하기)
     //한번에 다 읽을 수 있을 정도의 배열의 길이를 주어서 반복문 X
     ...
     try {
       // 생성의 역순으로 닫으면 언제나 안전하다. 
       if(bout != null) { bout.close();}
       if(bin != null) { bin.close();}
       if(con != null) {con.disconnect();}   //역순으로 닫아라 
     } catch(IOException e) {
       e.printStackTrace();
     }
   }

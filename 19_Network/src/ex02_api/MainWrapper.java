...
public class MainWrapper {
  ...
  public static void ex01() {
    /*
     * 한국 환경공단 에어코리아 대기오염정보 -  시도별 실시간 측정정보 조회
     * 1. 요청 주소 :http://apis.data.go.kr/B552584/ArpltnInforInqireSvc/getCtprvnRltmMesureDnsty
   ...
   sb.append(spec);
   sb.append("?serviceKey=").append(URLEncoder.encode(serviceKey, "UTF-8")); //UnsupportedEncordingException
  ...
  public static void ex02() {
    ...
    try {
      ...
      sbUrl.append("&sort=").append(sort);  //get 방식의 주소만들기 이다. get 방식의 HTTP메서드는 기본이기 때문에 생략가능
      
      url = new URL(sbUrl.toString());
      con = (HttpURLConnection) url.openConnection();
      
      // 요청 메소드 
      con.setRequestMethod("GET"); //반드시 대문자 GET/ 생략가능
      // 요청 헤더
      ...
      for(int i = 0,lenght = items.length(); i < lenght; i++) {
        ...
        System.out.println("작성날짜: " + item.getString("postdate"));
        System.out.println();
